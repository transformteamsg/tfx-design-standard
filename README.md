...
skills — `dxd:design` the loop, `dxd:standards` catalog mechanics, `dxd:copy`
voice & tone, `dxd:start` orientation, and others for critique, layout, motion, and
polish — the `dxd:evaluator` agent (which carries its own review procedure), and the
...
Run `/dxd:start` (or just say "onboard me") for a guided orientation — then ask Claude
