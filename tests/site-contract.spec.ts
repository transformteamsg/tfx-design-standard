...
const routes = [
  { name: "landing", path: "/" },
  { name: "get started", path: "/harness/get-started" },
  { name: "domains", path: "/domains" },
  { name: "platform domain", path: "/domains/platform" },
...
for (const width of mobileWidths) {
  test(`mobile chrome targets are at least 44px at ${width}px`, async ({ page }) => {
    await page.setViewportSize({ width, height: 900 });
    await open(page, "/harness/get-started");
    const primaryNavigation = page.getByRole("navigation", { name: "Primary" });

    await expectMinimumTarget(page.getByRole("button", { name: "Open navigation" }), 44);
    await expectMinimumTarget(
      primaryNavigation.getByRole("link", { name: "Domains", exact: true }),
      44
    );
    await expectMinimumTarget(
...
test.describe("reduced motion", () => {
  ...
  const animatedRoutes = [
    ...
    { name: "get started", path: "/harness/get-started", essentialText: "Read this page" },
    { name: "domains", path: "/domains", essentialText: "Domains" },
  ] as const;
