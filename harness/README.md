...
│   57 controls (latest ratchet 2026-07-08) ├─ setup (per-user tools + context)  │   DOM checks — non-skippable
...
│   ├── catalog.yaml         # TFX-DS catalog: 57 controls (always loaded)
...
CMP-6, LAY-1/7, IDN-2/3/4, CMP-4, CNT-4, CMP-8, CMP-9) — 57 controls, one file, consumed by both the harness (enforcement) and the
