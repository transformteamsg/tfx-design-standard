...
| 043 | Escape the bare `<date>` token in `controls/cmp-1.md` (042 follow-up F1) | P3 | XS | — | TODO (now fully specified; sole offender is the prose `<date>` at cmp-1.md:91 — the backticked tokens on lines 71/78/79 are already safe and must not be touched) |
| 044 | Close the type-scan TYP-1 blind spot — flag named `font-mono`/`font-serif` utilities | P1 | S–M | — | TODO (self-run finding L-2 — the check missed the font-mono gap) |
