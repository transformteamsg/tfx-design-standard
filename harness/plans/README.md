...
| 051 | App verification baseline — working ESLint flat config, `pnpm typecheck`, Vitest characterization tests for `lib/markdown-twin.ts`/`lib/catalog.ts`, CI workflow | P2 | M | — (independent; safe alongside 050) | DONE (revised & executed; branch advisor/051-verification-baseline; a first pass hit `eslint@10.6.0` + `eslint-config-next@16.2.9` — built for Next 16, crashed FlatCompat with "Converting circular structure to JSON"; fixed by pinning `eslint@^9.39` + `eslint-config-next@^15.5` to match `next@^15.3` — `pnpm lint` now runs, 0 errors/4 pre-existing warnings; `pnpm typecheck` clean; 17 Vitest characterization tests across `lib/markdown-twin.test.ts` + `lib/catalog.test.ts`, no `lib/` behaviour changes, no non-allowlisted field leak found in the public catalog projection; `.github/workflows/ci.yml` added [lint, typecheck, test, `check-standards.mjs`; `deploy.yml` untouched]; also fixed `eslint.config.mjs` to ignore the Next-generated `next-env.d.ts`, which otherwise breaks `pnpm lint` after any local `pnpm build`; full `pnpm lint && pnpm typecheck && pnpm test && pnpm build` exits 0) |
